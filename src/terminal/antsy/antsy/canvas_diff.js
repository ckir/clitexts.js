...
class ClearPoint {
    ...
    x, score = CLEAR_STARTING_SCORE, // how much better is it to do this clrtoeol vs without?
    chainScore = CLEAR_STARTING_SCORE) {
        this.attr = attr;
        this.bg = bg;
        this.x = x;
        this.score = score;
...
export function computeDiff(oldBuffer, newBuffer) {
    ...
    for (const y of range(0, oldBuffer.rows)) {
        ...
        for (const c of distance.clears)
            oldBuffer.clearToEndOfLine(c.x, y, c.attr);
...
function computeRowDistance(oldBuffer, oldy, newBuffer, newy = oldy) {
    ...
    const endRun = (x) => {
        ...
        if (run && x - run.x + 1 >= THRESHOLD_BLANKS)
            candidates.push(run);
        run = undefined;
    };
    ...
    let clears = [];
    while (candidates.length > 0 && candidates[0].score <= 0)
        candidates.shift();
...
function getDirtySpans(oldBuffer, newBuffer, y) {
    const spans = [];
    for (let left = 0; left < oldBuffer.cols; left++) {
        if (newBuffer.isSame(left, y, oldBuffer))
            continue;
...
function checkScroll(oldBuffer, newBuffer, s) {
    ...
    if (originalCost - newCost < THRESHOLD_SCROLL)
        return "";
...
function computeBlankDistance(buffer, y, blank) {
...
function changeAttr(buffer, attr, needForeground = true) {
    // if the current attr is -1, force both fg & bg to be generated.
    if (buffer.attr == -1)
        buffer.attr = 0xffff ^ attr;
    if (attr == buffer.attr)
        return "";
    if (!needForeground)
        attr = (attr & 0xff00) | (buffer.attr & 0xff);
...
function move(buffer, newX, newY) {
...
//# sourceMappingURL=canvas_diff.js.map